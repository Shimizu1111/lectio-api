...
import (
	...
	lectioapi "github.com/Shimizu1111/lectio/design"
...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[lectioapi] ", log.Ltime)
	}

	// Initialize the services.
	var (
		lectioSvc lectio.Service
	)
	{
		lectioSvc = lectioapi.NewLectio(logger)
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		lectioEndpoints *lectio.Endpoints
	)
	{
		lectioEndpoints = lectio.NewEndpoints(lectioSvc)
	}
	...
	case "localhost":
		{
			...
			handleHTTPServer(ctx, u, lectioEndpoints, &wg, errc, logger, *dbgF)
		}

		{
			...
			handleGRPCServer(ctx, u, lectioEndpoints, &wg, errc, logger, *dbgF)
		}
